...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.statusbar.setGeometry(QtCore.QRect(0, 170, 751, 50))
        font = QtGui.QFont()
        font.setFamily("Times New Roman")
        font.setPointSize(14)
        #font.setItalic(True)
    ...
    def runVgate(self, filename, sheetName, searchVgate, numVgates):
        ...
        tmp = []
        for p in range(numVgates):
            newY = []
            value = float(searchVgate[p])
    ...
    def saveFile(self):
        ...
        if not self.txt_vgate.text():
            ...
            ltd = []
            ltd = searchVgate.split(',')
            print(len(ltd))

            self.runVgate(self.filename, 0, ltd, len(ltd))
