...
# Form implementation generated from reading ui file '.\Final_App2_v3_Draw_Pic.ui'
#
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.txt_peaks = QtWidgets.QLineEdit(self.centralwidget)
        font = QtGui.QFont()
        font.setPointSize(18)
        self.txt_peaks.setFont(font)
        self.txt_peaks.setObjectName("txt_peaks")
        self.horizontalLayout_6.addWidget(self.txt_peaks)
        ...
        self.btn_cal.clicked.connect(lambda: self.calculate(False))
    ...
    def getPeakXY_Draw(self, df, deg1, deg2, nums, W):
        x = np.array(df["W"])
        y = np.array(df["Gp_W"])
        ...
        for k in range(deg1, deg2, 1):
            ...
            plt.plot(x_New[signal.argrelextrema(y_New, np.greater)[0]], y_New[signal.argrelextrema(y_New, np.greater)], '*', markersize=10, mfc='red', mec='k')
            plt.title(f'Polynomial order {k}')
        ...
        plt.suptitle('VBias ' + str(W), fontsize=20, y=1)

        global path
        path = 'Results/'
        if not os.path.isdir(path): os.makedirs(path)
        plt.savefig(os.path.join(path, "VBais " + str(W) + ".png"))
    ...
    def draw(self):
        if self.txt_peaks.text() != '' :
            self.calculate(True)
            peak_Vlist = self.txt_peaks.text()
            peak_Vlist = peak_Vlist.rstrip(',')
            peak_Vlist = list(peak_Vlist.split(","))
            peak_Vlist = [float(i.strip()) for i in peak_Vlist]
            j = 1

            # 3. output results
            try:
                wb = xlsxwriter.Workbook('result_images.xlsx')
                ws = wb.add_worksheet('Sheet1')
                for i in range(len(init._V)):
                    if init._V[i] in peak_Vlist:
                        ws.insert_image('A' + str(j), os.path.join(path, "VBais " + str(init._V[i]) + ".png"))
                        j += 30
                wb.close()
            ...
            print("Peaks(V) is null.")

    def loadFile(self):
        global excelPath
        excelPath = QFileDialog.getOpenFileName(filter="Excel File (*.xlsx *.xls)")[0]
        if excelPath != '':
            try :
                self.txt_file.setText(excelPath)

                # Definite Value
                global init
                init = initVal()
                # Get dataframe dataset
                init.df1 = pd.read_excel(excelPath, header=None)  # mangle_dupe_cols problems
                init.df1.columns = init.df1.iloc[0]
                init.df1 = init.df1[1:]
                init.df1 = init.df1.set_index(list(init.df1)[0])
                ...
                # Set default Peak V
                self.txt_peaks.setText(str(init._V[0]))
            except:
                print("An error occurred while loading the file")
                pass

    def calculate(self, boolVal):
        global isDraw
        isDraw = boolVal
        if excelPath != '' :
            if (int(self.txt_deg1.text()) < 4):  # limit degree > 3
                ...
                for i in range(len(init._V)):
                    ...
                    if init._V[i] in peak_Vlist :
                        self.getPeakXY_Draw(pd.DataFrame(Gp_W_Results), deg1, deg2, sampleNums, init._V[i])
                    else:
                ...
                try:
                    ...
                    print("Output excel file error")
                    pass
        else:
            print("Please load the input file. ")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_9.setText(_translate("MainWindow", "Plot Peaks (V) :"))
